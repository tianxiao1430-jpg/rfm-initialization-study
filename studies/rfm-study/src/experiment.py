...
def run(config,path):
    ...
    env=rfm.environment();env['sources']={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [Path(__file__),ROOT/'src/components.py',ROOT/'vendor/rfm.py']}
