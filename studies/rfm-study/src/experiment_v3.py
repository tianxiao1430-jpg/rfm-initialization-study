...
from directions_v3 import initial_direction
...
def run(config,path):
    ...
    env=rfm.environment();env['sources']={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in [Path(__file__),ROOT/'src/components.py',ROOT/'vendor/rfm.py',ROOT/'src/experiment.py',ROOT/'src/directions_v2.py',ROOT/'src/directions_v3.py']}
    if c['kind']=='custom':env['initializer_sha256']=hashlib.sha256((ROOT/'initializations'/c['amount']).read_bytes()).hexdigest()
